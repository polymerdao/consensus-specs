...
Full nodes SHOULD provide the `LightClientFinalityUpdate` with higher `sync_committee` participation than the one defined in general `LightClientUpdate`.  Referring to `sync-protocol.md`, the number of participants SHOULD provide super majority (2/3) of the `sync_committee` for the `LightClientFinalityUpdate` to be considered.
